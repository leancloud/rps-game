...
import { actions, Event as RPSGameEvent, events, filter, initialStates } from "../rps-game-rules";
...
export default class RPSGame extends defineGame({
  ...
  protected start = async () => {
    ...
    this.masterClient.setRoomOpened(false);
    // 向客户端广播游戏开始事件
    this.dispatchEvent(RPSGameEvent.GAME_START);
    // 监听 player 离开游戏事件
    listen(this.masterClient, PlayEvent.PLAYER_ROOM_LEFT).then(() => this.dispatchEvent(RPSGameEvent.PLAYER_LEFT));
  }
}
